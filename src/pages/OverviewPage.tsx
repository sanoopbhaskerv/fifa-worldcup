    ...
    <div className="page">
      ...
      {featured && (
        <Link className="hero-match" to={`matches/${featured.id}`}>
          <div className="hero-match__top"><span className={`status status--${featured.status.toLowerCase()}`}>{featured.status === "LIVE" ? `LIVE${featured.minute ? ` · ${featured.minute}'` : ""}` : "NEXT MATCH"}</span><span>{featured.stage} · {featured.round}</span></div>
          <div className="hero-match__teams">
            ...
            <div className="hero-match__score">
              {featured.homeScore !== undefined ? <strong>{featured.homeScore} <span>:</span> {featured.awayScore}</strong> : <><strong>{formatKickoff(featured.kickoff)}</strong><span>{formatDate(featured.kickoff)}</span></>}
            </div>
