...
describe("CompetitionPicker", () => {
  it("searches and selects a competition", async () => {
    ...
    render(<CompetitionPicker open competitions={competitionCatalog} currentId="world-cup" favorites={[]} recents={[]} onClose={vi.fn()} onSelect={onSelect} onToggleFavorite={vi.fn()} />);
    ...
    expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ id: "premier-league", activeEditionId: "2025-26" }));
  });

  it("filters by region or confederation", async () => {
    const user = userEvent.setup();
    render(<CompetitionPicker open competitions={competitionCatalog} currentId="world-cup" favorites={[]} recents={[]} onClose={vi.fn()} onSelect={vi.fn()} onToggleFavorite={vi.fn()} />);
