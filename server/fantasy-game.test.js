...
describe("fantasy game API", () => {
  ...
  it("tests the external AI provider without creating a host message draft", async () => {
    const fetchMock = vi.fn(async () => ({
      ok: true,
      json: async () => ({
        choices: [
          {
            message: {
              content: JSON.stringify({
                title: "Live AI provider works",
                body: "The leaderboard pulse can now be generated by the configured provider.",
              }),
            },
          },
        ],
    ...
    const response = await handleApiRequest({
      method: "POST",
      url: "/api/fantasy/admin/ai-messages/provider-test",
      body: JSON.stringify({ actorId: "p-sanoop" }),
      env: {
        FANTASY_AI_PROVIDER_URL: "https://ai.example/chat/completions",
        FANTASY_AI_API_KEY: "test-key",
        FANTASY_AI_MODEL: "free-small",
    ...
    expect(response.body.game.aiMessages).toEqual([]);
    expect(response.body.game.auditRecords.at(-1)).toMatchObject({
